...
(function(){
  ...
  gsap.fromTo('#keyVisualImg', 1.6, {opacity: 0}, {opacity: 1}).delay(.2);
  gsap.fromTo('#keyVisualImg', 1, {transform: 'scale(.3)', transformOrigin: 'center'}, {transform: 'scale(1)'});

  gsap.fromTo('#keyVisualImgM', 1.6, {opacity: 0}, {opacity: 1}).delay(.2);
  gsap.fromTo('#keyVisualImgM', 1, {transform: 'scale(.3)', transformOrigin: 'center'}, {transform: 'scale(1)'});

  gsap.fromTo('.top-video-text h1, .top-video-text h6', 1.5, {opacity: 0}, {opacity: 1}).delay(.6);
...
function setParallaxItemsMarginTop(windowHeight) {
  $('.parallax-item').each((idx, el) => {
    ...
    const margin = parallax.diff * getArrivalRatio($el, windowHeight, parallax.delay);
...
function setPositionalOpacities(windowHeight) {
  $('.section-container h1:not(.not-fadein), .section-container h2:not(.not-fadein), .section-container h3:not(.not-fadein), .section-container h4:not(.not-fadein), .section-container h5:not(.not-fadein), .section-container h6:not(.not-fadein), .section-container p:not(.not-fadein)').each((idx, el) => {
    const $el = $(el);
...
$(document).on('click', '#bgLocation1', function (e){
  ...
  $(this).closest('.bg-gallery').removeClass('type-studio').addClass('type-location');
})
$(document).on('click', '#bgStudio1', function (e){
  ...
  $(this).closest('.bg-gallery').addClass('type-studio').removeClass('type-location');
});
...
$(document).on('click', '.v-le .chip-color:first-of-type', function (e) {
  ...
  $('.car-swiper .swiper-slide img, .car-swiper .swiper-pagination-bullet img').each(function(){
    var $this = $(this);
    $this.attr('src',$this.attr('src').replace('/gallery_02_','/gallery_01_'))
  });
...
$(document).on('click', '.v-le .chip-color:nth-of-type(2)', function (e) {
  ...
  $('.car-swiper .swiper-slide img, .car-swiper .swiper-pagination-bullet img').each(function(){
    var $this = $(this);
    $this.attr('src',$this.attr('src').replace('/gallery_01_','/gallery_02_'))
  });
  $('#colorName').html($colorName);
});
...
$(document).on('click', '.v-23my .type-exterior .chip-color', function (e) {
  ...
  if ($(this).hasClass('on')) {
    ...
    $(this).addClass('on');
    $('.car-swiper1 .swiper-slide img, .car-swiper1 .swiper-pagination-bullet img').each(function(){
      var $this = $(this);
...
$(function () {
  if ($('.position-wrap').length == 1) {
    if ($(window).width() < tbl) {
      ...
      let hdServiceSlide = new Swiper('.position-wrap .swiper-container', swpOpt);
    }
    ;
  }
});